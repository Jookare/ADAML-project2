...
class TimeSeriesDataset(Dataset):
    ...
    def __init__(self, df, window_size=7, horizon=0, stride=1):
    ...
    def _sequencing(self, window_size, horizon, stride):
        """
        Arguments:
        ...
        input = np.empty(
            (len(sequences), window_size, n_features + 2), dtype=np.float32
        )
